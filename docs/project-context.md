...
金融客服的意图识别采用有边界的分层决策，不能退化为单个 Prompt 分类，也不能让分类模型直接决定权限或调用任意工具。固定层次为：

```text
L0 已验证身份、租户、渠道、消息和会话上下文组装
→ L1 安全、高风险和强制转人工意图
→ L2 可解释的高精度确定性规则
→ L3 受限分类表与 JSON Schema 约束的语义模型识别
→ L4 规则、候选差距、实体完整性和风险信号融合及置信度校准
→ L5 Policy Router 执行 Agent、Skill、权限、版本、预算和审批决策
```

以下约束在后续实现中不可省略：

- 意图分类采用可版本化的层级、多意图分类表，并声明正反例、必需实体、风险等级、鉴权要求、目标能力、澄清和回退策略。
- 分类模型只产生候选意图、实体、缺失实体和风险信号；最终 Agent/Skill 路由必须由 Policy Router 根据可信身份和 Registry 状态决定。
- 安全、租户、权限和人工审批结果优先于语义模型结果，模型不能覆盖拒绝决策。
- 最终置信度不能直接采用 LLM 自报分数，必须在版本化黄金集上结合规则精度、候选差距、实体完整性和真实错误分布进行校准。
- 高置信度请求可以自动路由；信息缺失或中等置信度请求应澄清；低置信度、未知和高风险请求应拒答或转人工。具体门限由当前系统评测数据确定。
- 多意图问题由 Planner 拆分为有依赖的任务；知识条款走 RAG，利率、额度、征信、审批和个性化计算等动态数据走授权 MCP，不得互相替代。
- 意图分类表版本、决策来源、候选、实体、最终路由、拒绝或澄清原因必须关联 `trace_id` 并进入 Run 快照或可回放事件。
- 后续 Harness 必须覆盖规则与模型冲突、相似意图混淆、缺失实体、上下文指代、多意图、未知意图、跨租户、越权和模型不可用场景。
